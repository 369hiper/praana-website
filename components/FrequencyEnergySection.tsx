...
const InfoCard: React.FC<{
  ...
  return (
    <div
      ref={cardRef}
      className={`relative group p-6 rounded-2xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-slate-700/50 backdrop-blur-xl transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}
    >
...
const ProductPreviewCard: React.FC<{
  ...
  return (
    <Link
      to={route}
      ref={cardRef as any}
      className={`group relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700/50 transition-all duration-700 hover:border-praana-accent/50 hover:shadow-[0_0_40px_rgba(20,184,166,0.2)] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
        }`}
    >
      {/* Glow effect */}
